...
class InsightForgeResult:
    ...
    def to_text(self) -> str:
        ...
        if self.semantic_facts:
            text_parts.append(f"\n### [Key Facts] (cite these original texts in the report)")
...
class PanoramaResult:
    ...
    def to_text(self) -> str:
        ...
        text_parts = [
            f"## Breadth Search Results (Full Panorama)",
            f"Query: {self.query}",
            f"\n### Statistics",
        ...
        if self.active_facts:
            text_parts.append(f"\n### [Active Facts] (simulation result original text)")
    ...
    def to_text(self) -> str:
        ...
        if self.key_quotes:
            ...
            for quote in self.key_quotes:
                ...
                # Filter out garbage content containing question numbers (1-9)
                skip = False
                for d in '123456789':
                ...
                if skip:
                    continue
                # Truncate overly long content (by sentence, not hard truncation)
...
class ZepToolsService:
    ...
    def search_graph(
        ...
        Falls back to local keyword matching if Zep Cloud's search API is unavailable.
    ...
    def _local_search(
        ...
        def match_score(text: str) -> int:
            """Calculate text match score against query"""
            if not text:
                return 0
    ...
    def get_node_edges(self, graph_id: str, node_uuid: str) -> List[EdgeInfo]:
        ...
        Fetches all graph edges and filters for those connected to the specified node
    ...
    def get_entity_summary(
        ...
        Searches all information related to this entity and generates a summary
    ...
    def get_simulation_context(
        ...
        Aggregates all information relevant to the simulation requirement
        ...
        # Filter entities with specific types (not just "Entity" nodes)
        entities = []
        for node in all_nodes:
    ...
    def insight_forge(
        ...
        5. Aggregates all results to produce deep insights
        ...
        # Step 3: Extract relevant entity UUIDs from edges; only get those entities (not all nodes)
        entity_uuids = set()
        ...
        for uuid in list(entity_uuids):  # Process all entities, no truncation
            ...
            try:
                ...
                if node:
                    ...
                    # Get all facts for this entity (no truncation)
                    related_facts = [
    ...
    def _generate_sub_queries(
        ...
        Decomposes a complex question into multiple independently retrievable sub-queries
        """
        system_prompt = """You are an expert question analyst. Your task is to decompose a complex question into multiple sub-questions that can be independently observed in a simulation world.
...
Return a JSON list of sub-questions."""
    ...
    def panorama_search(
        ...
        Gets a full panorama view, including all related content and historical/expired information:
        ...
        This tool is suited for scenarios requiring a full picture of events or tracking evolution over time.
    ...
    def interview_agents(
        ...
        5. Aggregates all interview results into an interview report

        [Important] This feature requires the simulation environment to be running (OASIS environment not yet closed)
        ...
        - Need real answers from simulation agents (not LLM simulation)
        ...
        if not profiles:
            logger.warning(f"No persona file found for simulation {simulation_id}")
            result.summary = "No agent persona file found for interview"
            return result
        ...
        try:
            # Build batch interview list (no platform = both platforms)
            interviews_request = []
            for agent_idx in selected_indices:
                interviews_request.append({
                    "agent_id": agent_idx,
                    "prompt": optimized_prompt  # Use optimized prompt
                    # Not specifying platform; API interviews on both twitter and reddit
                })
            
            logger.info(f"Calling batch interview API (dual platform): {len(interviews_request)} agents")
            
            # Call SimulationRunner batch interview method (no platform = both platforms)
            ...
            for i, agent_idx in enumerate(selected_indices):
                ...
                # Strategy 2 (supplementary): long text within correctly paired quotes
                if not key_quotes:
            ...
            result.summary = f"Interview failed: {str(e)}. Simulation environment may have shut down, please ensure OASIS environment is running."
            return result
        except Exception as e:
    ...
    def _clean_tool_call_response(response: str) -> str:
        """Clean up JSON tool-call wrappers from agent responses, extracting actual content"""
    ...
    def _select_agents_for_interview(
        ...
        Returns:
            tuple: (selected_agents, selected_indices, reasoning)
                ...
                - selected_indices: List of selected agent indices (for API call)
        ...
        system_prompt = """You are an expert interview planner. Your task is to select the most suitable agents for interview from a list of simulation agents based on the interview requirement.

Selection criteria:
1. The agent's identity/profession is relevant to the interview topic
2. The agent may hold unique or valuable perspectives
3. Select diverse viewpoints (e.g., supporters, opponents, neutral parties, experts)
...
Select up to {max_agents} most suitable agents for the interview and explain your selection rationale."""
...
Generate 3-5 interview questions."""
        ...
        try:
            ...
            return [
                ...
                "How has this event affected you or the group you represent?",
                "In your opinion, how should this issue be resolved or improved?"
            ]
    ...
    def _generate_interview_summary(
        ...
        system_prompt = """You are a professional news editor. Based on the responses from multiple interviewees, generate an interview summary in English.
