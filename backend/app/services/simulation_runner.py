...
Runs simulation in the background, records each Agent's actions, and supports real-time status monitoring.
"""
...
# Flag to track whether cleanup function has been registered
...
class SimulationRunner:
    ...
    def start_simulation(
        ...
        # Determine which script to run (scripts located in backend/scripts/)
        if platform == "twitter":
        ...
        try:
            ...
            # This fixes issues where third-party libs (e.g. OASIS) open files without specifying encoding
            env = os.environ.copy()
            env['PYTHONUTF8'] = '1'  # Python 3.7+: makes all open() default to UTF-8
            env['PYTHONIOENCODING'] = 'utf-8'  # Ensures stdout/stderr use UTF-8

            # Set working directory to simulation dir (databases etc. are created here)
            # Use start_new_session=True to create a new process group, allowing os.killpg to terminate all children
            process = subprocess.Popen(
                ...
                start_new_session=True,  # New process group; ensures all children terminate when server closes
            )
    ...
    def _monitor_simulation(cls, simulation_id: str):
        ...
        try:
            ...
            # Process ended, do a final log read
            if os.path.exists(twitter_actions_log):
                        ...
                        try:
                            ...
                            if "event_type" in action_data:
                                ...
                                if event_type == "simulation_end":
                                    ...
                                    # If only one platform ran, only check that one
                                ...
                                # Update round info (from round_end event)
                                    ...
                                    # Overall round = max of two platforms
                                    if round_num > state.current_round:
                                        state.current_round = round_num
                                    # Overall time = max of two platforms
    ...
    def _check_all_platforms_completed(cls, state: SimulationRunState) -> bool:
        ...
        Determines whether a platform is enabled by checking if the corresponding actions.jsonl exists.
    ...
    def _terminate_process(cls, process: subprocess.Popen, simulation_id: str, timeout: int = 10):
        """
        Cross-platform process termination, including child processes.
        ...
        if IS_WINDOWS:
            ...
            # /F = force, /T = terminate tree (including children)
            ...
            # Since start_new_session=True was used, pgid equals the main process PID
        ...
        with open(file_path, 'r', encoding='utf-8') as f:
            for line in f:
                ...
                try:
                    ...
                    # Skip non-action entries (e.g. simulation_start, round_start, round_end events)
                    ...
                    # Get platform: prefer platform field in record, fall back to default_platform
    ...
    def get_all_actions(
        ...
        # If per-platform files not found, try legacy single-file format
        if not actions:
            actions_log = os.path.join(sim_dir, "actions.jsonl")
            actions = cls._read_actions_from_file(
                actions_log,
                default_platform=None,  # Legacy format files should have the platform field
                platform_filter=platform,
                agent_id=agent_id,
                round_num=round_num
            )
    ...
    def cleanup_all_simulations(cls):
        ...
        # Check if there is anything to clean (avoid printing useless logs for empty process dicts)
        has_processes = bool(cls._processes)
        has_updaters = bool(cls._graph_memory_enabled)
        
        if not has_processes and not has_updaters:
            return  # Nothing to clean, silently return
    ...
    def register_cleanup(cls):
        ...
        # In Flask debug mode, only register cleanup in the reloader subprocess (the process actually running the app)
        ...
        # In debug mode, only register in the reloader subprocess; in non-debug mode, always register
        ...
        def cleanup_handler(signum=None, frame=None):
            ...
            # Only log if there are processes to clean
            if cls._processes or cls._graph_memory_enabled:
                logger.info(f"Received signal {signum}, starting cleanup...")
            cls.cleanup_all_simulations()
            
            # Call the original signal handler to let Flask exit gracefully
    ...
    def interview_agent(
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None: Interview on both platforms for dual-platform simulations, returns merged result
        ...
        if not ipc_client.check_env_alive():
            raise ValueError(f"Simulation environment is not running or has shut down, cannot execute interview: {simulation_id}")
    ...
    def interview_agents_batch(
        ...
        Args:
            ...
            platform: Default platform (optional, overridden by each interview item's platform)
                ...
                - None: Interview each agent on both platforms in dual-platform simulations
            timeout: Timeout in seconds
    ...
    def close_simulation_env(
        ...
        Sends a close-environment command to the simulation, causing it to gracefully exit the command-waiting mode
    ...
    def get_interview_history(
        ...
        # If multiple platforms queried, cap the total count
