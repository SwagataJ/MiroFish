...
Converts entities from the Zep graph to the Agent Profile format required by the OASIS simulation platform

Improvements:
1. Call Zep retrieval to further enrich node information
2. Optimized prompts to generate very detailed personas
3. Differentiates between individual entities and abstract group entities
"""
...
class OasisProfileGenerator:
    ...
    Converts entities from the Zep graph to the Agent Profile format required by OASIS simulation
    
    Optimization features:
    1. Call Zep graph retrieval to obtain richer context
    ...
    def _search_zep_for_entity(self, entity: EntityNode) -> Dict[str, Any]:
        """
        Get rich information about an entity using Zep graph hybrid search
        
        Zep has no built-in hybrid search endpoint; edges and nodes are searched separately and results merged.
        ...
        # graph_id must be set to perform search
    ...
    def _build_entity_context(self, entity: EntityNode) -> str:
        ...
        # 3. Add detailed info from related nodes
        ...
        if zep_results.get("facts"):
            # Deduplicate: exclude already-included facts
        ...
        for attempt in range(max_attempts):
            try:
                response = self.client.chat.completions.create(
                    ...
                    # Do not set max_tokens; let the LLM output freely
                )
    ...
    def _fix_truncated_json(self, content: str) -> str:
        ...
        # Simple check: if nothing follows the last quote, the string may be truncated
    ...
    def _try_fix_json(self, content: str, entity_name: str, entity_type: str, entity_summary: str = "") -> Dict[str, Any]:
        ...
        if json_match:
            ...
            # 3. Handle newline-in-string issues
    ...
    def _get_system_prompt(self, is_individual: bool) -> str:
        """Get system prompt"""
        base_prompt = "You are an expert in generating social media user personas. Generate detailed, realistic personas for opinion simulation that best reflect existing real-world conditions. Must return valid JSON format; all string values must not contain unescaped newlines."
        return base_prompt
    ...
    def _build_individual_persona_prompt(
        ...
        return f"""Generate a detailed social media user persona for the entity, best reflecting existing real-world conditions.
...
2. persona: Detailed persona description (~2000 words, plain text), including:
   ...
   - Personal memory (important part of the persona: describe this individual's connection to the event and their existing actions and reactions in it)
...
"""
    ...
    def _build_group_persona_prompt(
        ...
        """Build detailed persona prompt for a group/institution entity"""
        
        attrs_str = json.dumps(entity_attributes, ensure_ascii=False) if entity_attributes else "N/A"
        context_str = context[:3000] if context else "No additional context"
        
        return f"""Generate a detailed social media account profile for an organization/group entity, best reflecting existing real-world conditions.
...
2. persona: Detailed account profile description (~2000 words, plain text), including:
   ...
   - Organizational memory (important part of the persona: describe this organization's connection to the event and its existing actions and reactions in it)
3. age: Fixed as 30 (virtual age for organization accounts)
4. gender: Fixed as "other" (organizations use "other" to indicate non-individual)
...
- The organization account's communications must fit its identity positioning"""
    ...
    def _generate_profile_rule_based(
        ...
        """Generate basic persona using rule-based logic"""
        ...
        if entity_type_lower in ["student", "alumni"]:
            ...
            return {
                ...
                "mbti": "ISTJ",  # Institution style: methodical and conservative
                "country": "China",
                "profession": "Media",
    ...
    def generate_profiles_from_entities(
        ...
        Args:
            ...
            realtime_output_path: File path for real-time writes (if provided, writes each profile as it completes)
        ...
        completed_count = [0]  # Use list so closure can modify it
        lock = Lock()
        ...
        def save_profiles_realtime():
            ...
            with lock:
                # Filter for already-generated profiles
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=parallel_count) as executor:
            ...
            for future in concurrent.futures.as_completed(future_to_entity):
                ...
                try:
                    ...
                    # Write to file immediately (even for fallback personas)
                    save_profiles_realtime()
    ...
    def _save_twitter_csv(self, profiles: List[OasisAgentProfile], file_path: str):
        ...
        - user_id: User ID (0-based, by CSV order)
        - name: User real name
    ...
    def _normalize_gender(self, gender: Optional[str]) -> str:
        ...
        # Normalize gender strings to English
        gender_map = {
            "男": "male",
            "女": "female",
            "机构": "other",
