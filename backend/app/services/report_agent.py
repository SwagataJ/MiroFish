...
class ReportLogger:
    ...
    def log(
        ...
        Args:
            ...
            details: Detail content dictionary, not truncated
    ...
    def log_section_content(
        ...
        """Log section content generation complete (content only, not the full section completion)"""
        self.log(
    ...
    def log_section_full_complete(
        ...
        The frontend should listen for this log to determine when a section is truly complete and get the full content
        """
        self.log(
...
[InsightForge - Powerful Deep Retrieval]
Our most powerful retrieval function, designed for deep analysis. It will:
1. Automatically decompose your question into multiple sub-queries
...
[PanoramaSearch - Full Panorama View]
...
2. Distinguish currently active facts from historical/expired ones
3. Help you understand how public opinion evolved

[When to use]
- Need to understand the full development arc of an event
- Need to compare public opinion across different stages
...
Lightweight fast retrieval tool, suited for simple, direct information queries.
...
[InterviewAgents - Real Agent Interviews (Dual Platform)]
Calls the OASIS simulation environment's interview API to conduct real interviews with running simulation agents!
This is NOT an LLM simulation — it calls the actual interview interface to get agents' raw responses.
By default, interviews run simultaneously on Twitter and Reddit for more comprehensive perspectives.

How it works:
1. Auto-reads persona files to learn about all simulation agents
...
5. Aggregates all interview results for multi-perspective analysis

[When to use]
- Need to understand event perspectives from different role viewpoints (students? media? officials?)
- Need to collect opinions and stances from multiple parties
- Need real responses from simulation agents (from the OASIS environment)
- Want to make the report more vivid with "interview transcripts"

[Returns]
- Identity information of interviewed agents
- Each agent's responses on Twitter and Reddit
...
[Core Concept]
We built a simulation world and injected a specific "simulation requirement" as a variable. The evolution of the simulation world is a prediction of what may happen in the future. What you are observing is not "experimental data" but a "rehearsal of the future."
...
- ✅ Agent behavior and statements in the simulation world are predictions of future human behavior
...
- No sub-sections needed; write complete content directly in each section
...
From a "god's-eye view," examine this rehearsal of the future:
...
1. [Must call tools to observe the simulation world]
   ...
   - Call tools at least 3 times per section (max 5) to observe the simulation world, which represents the future

2. [Must cite Agent's original statements and actions]
   - Agent statements and behavior are predictions of future human behavior
   - Use citation format in the report to present these predictions, e.g.:
     > "A group of people will say: original content..."
...
3. [Language consistency - write the entire report in English]
   ...
   - The report must be written entirely in English regardless of the language of the source material
   - When citing tool results, render them in natural, fluent English
...
- ❌ No Markdown headings inside a section (#, ##, ###, #### etc.)
- ❌ Do not add the section title at the start of the content
- ✅ The section title is added automatically by the system; just write the body text
- ✅ Use **bold**, paragraph breaks, block quotes, and lists to organize content — but no headings

[Correct example]
```
This section analyzes the public opinion propagation dynamics of the event. Through deep analysis of the simulation data, we find...

**Initial Ignition Phase**

Platform X served as the first scene of public opinion, carrying the core function of first publication:

> "Platform X contributed 68% of initial volume..."
...
[Tool usage advice - mix different tools, do not rely on only one]
- insight_forge: Deep insight analysis — auto-decomposes the question and retrieves facts and relationships across multiple dimensions
- panorama_search: Wide-angle panorama search — understand the full picture, timeline, and evolution of an event
...
Each reply must do exactly one of the following two things (not both):
...
The system will execute the tool and return the result. You must not write tool results yourself.

Option B - Output final content:
When you have gathered enough information via tools, start your reply with "Final Answer:" and output the section content.
...
2. Cite original text extensively to showcase simulation results
3. Use Markdown formatting (but no headings):
   - Use **bold text** to highlight key points (in place of sub-headings)
   - Use lists (- or 1. 2. 3.) to organize points
   - Use blank lines to separate paragraphs
   - ❌ No heading syntax: #, ##, ###, #### etc.
4. [Citation format - must be a standalone paragraph]
   Citations must stand alone with a blank line before and after; they cannot be embedded in a paragraph:
...
- ❌ No headings at all (#, ##, ###, #### are all forbidden)
...
- If information is sufficient: start with "Final Answer:" and output section content (must cite the text above)
...
REACT_TOOL_LIMIT_MSG = (
    ...
    'Please immediately output the section content based on the information already retrieved, starting with "Final Answer:".'
)
...
2. Answer directly; avoid lengthy reasoning dissertations
...
class ReportAgent:
    ...
    def __init__(
        ...
        # Logger (initialized in generate())
        self.report_logger: Optional[ReportLogger] = None
        # Console logger (initialized in generate())
        ...
        return {
            "insight_forge": {
                ...
                "parameters": {
                    ...
                    "report_context": "Context of the current report section (optional, helps generate more precise sub-queries)"
                }
            },
            "panorama_search": {
    ...
    def _execute_tool(self, tool_name: str, parameters: Dict[str, Any], report_context: str = "") -> str:
        ...
        try:
            ...
            # ========== Legacy tools for backwards compatibility (internally redirected) ==========
    ...
    def _parse_tool_calls(self, response: str) -> List[Dict[str, Any]]:
        ...
        # Only tried when format 1 did not match, to avoid false matches in body text
    ...
    def _generate_section_react(
        ...
        """
        Generate a single section using ReACT mode
        ...
        if previous_sections:
            ...
            for sec in previous_sections:
                # At most 4000 chars per section
        ...
        conflict_retries = 0  # Consecutive conflicts
        used_tools = set()  # Track called tools
        ...
        for iteration in range(max_iterations):
            ...
            if response is None:
                ...
                # If more iterations remain, retry
                if iteration < max_iterations - 1:
                    messages.append({"role": "assistant", "content": "(empty response)"})
                    messages.append({"role": "user", "content": "Please continue generating content."})
                    continue
                # Last iteration returned None, break to force-finish
                break
            ...
            # ── Conflict: LLM output both a tool call and a Final Answer ──
            if has_tool_calls and has_final_answer:
                ...
                if conflict_retries <= 2:
                    # First two: discard, ask LLM to reply again
                    ...
                    # Third: downgrade, truncate to first tool call, force execute
                    logger.warning(
                        f"Section {section.title}: {conflict_retries} consecutive conflicts, "
                        "downgrading to execute first tool call only"
                    )
            ...
            if has_final_answer:
                # Insufficient tool calls, require more
                if tool_calls_count < min_tool_calls:
                    messages.append({"role": "assistant", "content": response})
                    unused_tools = all_tools - used_tools
                    unused_hint = f"(These tools have not been used yet, consider trying: {', '.join(unused_tools)})" if unused_tools else ""
                    messages.append({
                        "role": "user",
            ...
            if has_tool_calls:
                # Tool budget exhausted, require Final Answer
                ...
                call = tool_calls[0]
                if len(tool_calls) > 1:
                    logger.info(f"LLM attempted {len(tool_calls)} tool calls, executing only first: {call['name']}")
            ...
            # Accept directly as final answer
            logger.info(f"Section {section.title}: no 'Final Answer:' prefix, adopting LLM output directly (tool calls: {tool_calls_count})")
        ...
        if response is None:
            logger.error(f"Section {section.title}: LLM returned None during forced finish")
            final_answer = "(Section generation failed: LLM returned empty response, please try again)"
    ...
    def generate_report(
        ...
        Each section is saved to disk immediately after generation without waiting for the full report.
        File structure:
        reports/{report_id}/
        ...
        try:
            ...
            try:
                ...
                pass  # Ignore errors saving the failure state
...
class ReportManager:
    ...
    Responsible for persisting and retrieving reports
    ...
    def get_console_log(cls, report_id: str, from_line: int = 0) -> Dict[str, Any]:
        ...
        This is the console output log (INFO, WARNING, etc.) generated during report creation,
        distinct from the structured log in agent_log.jsonl.
        
        Args:
            report_id: Report ID
            from_line: Line number to start reading from (for incremental fetch, 0 = from beginning)
    ...
    def _clean_section_content(cls, content: str, section_title: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if heading_match:
                ...
                # Section headings are added by the system, so content should have no headings
    ...
    def get_generated_sections(cls, report_id: str) -> List[Dict[str, Any]]:
        ...
        Returns info about all saved section files
        """
        folder = cls._get_report_folder(report_id)
    ...
    def assemble_full_report(cls, report_id: str, outline: ReportOutline) -> str:
        ...
        # Post-process: clean up heading issues across the whole report
    ...
    def _post_process_report(cls, content: str, outline: ReportOutline) -> str:
        ...
        2. Keep report main title (#) and section titles (##), remove deeper headings (###, ####, etc.)
        ...
        while i < len(lines):
            ...
            if heading_match:
                ...
                # Check if this is a duplicate heading (same content within 5 consecutive lines)
                is_duplicate = False
                ...
                if is_duplicate:
                    # Skip duplicate heading and any following empty line
                    i += 1
                ...
                # - # (level=1) Only keep as report main title
                # - ## (level=2) Keep as section titles
                # - ### and deeper (level>=3) Convert to bold text
                
                if level == 1:
                    if title == outline.title:
                        # Keep as report main title
                    ...
                    # Convert ### and deeper headings to bold text
                    processed_lines.append(f"**{title}**")
                    processed_lines.append("")
                    prev_was_heading = False
                ...
                # Keep only one empty line after headings
        ...
        # Clean up consecutive empty lines (keep at most 2)
        result_lines = []
        empty_count = 0
    ...
    def delete_report(cls, report_id: str) -> bool:
        ...
        # Backwards compatibility: delete individual file
        deleted = False
