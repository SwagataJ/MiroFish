...
def get_graph_entities(graph_id: str):
    ...
    try:
        if not Config.ZEP_API_KEY:
            return jsonify({
                "success": False,
                "error": get_text("zep_not_configured")
            }), 500
...
def get_entity_detail(graph_id: str, entity_uuid: str):
    ...
    try:
        ...
        if not entity:
            return jsonify({
                "success": False,
                "error": get_text("entity_not_found", entity_uuid=entity_uuid)
            }), 404
...
def create_simulation():
    ...
    try:
        ...
        if not project_id:
            return jsonify({
                "success": False,
                "error": get_text("provide_project_id")
            }), 400
        ...
        if not project:
            return jsonify({
                "success": False,
                "error": get_text("project_not_found", project_id=project_id)
            }), 404
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": get_text("sim.graph_not_built")
            }), 400
...
def _check_simulation_prepared(simulation_id: str) -> tuple:
    ...
    Note: Run scripts (run_*.py) remain in backend/scripts/ directory; no longer copied to simulation dir.
    ...
    try:
        ...
        # - preparing: if config_generated=True it means completion
        # - running: currently running, preparation was done earlier
        # - completed: run finished, preparation was done earlier
        # - stopped: stopped, preparation was done earlier
...
def prepare_simulation():
    ...
    try:
        ...
        if not simulation_id:
            return jsonify({
                "success": False,
                "error": get_text("sim.provide_simulation_id")
            }), 400
        ...
        if not state:
            return jsonify({
                "success": False,
                "error": get_text("sim_not_found", simulation_id=simulation_id)
            }), 404
            ...
            if is_prepared:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "message": get_text("sim.already_prepared"),
        ...
        if not project:
            return jsonify({
                "success": False,
                "error": get_text("project_not_found", project_id=state.project_id)
            }), 404
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": get_text("sim.missing_sim_requirement")
            }), 400
        ...
        def run_prepare():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message=get_text("sim.prepare_env_started")
                )
                ...
                def progress_callback(stage, progress, message, **kwargs):
                    ...
                    stage_names = {
                        "reading": get_text("sim.stage_reading"),
                        "generating_profiles": get_text("sim.stage_generating_profiles"),
                        "generating_config": get_text("sim.stage_generating_config"),
                        "copying_scripts": get_text("sim.stage_copying_scripts")
                    }
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "preparing",
                "message": get_text("sim.prepare_started"),
...
def get_prepare_status():
    ...
    1. By task_id to query ongoing task progress
    2. By simulation_id to check if preparation is already complete
    ...
    try:
            ...
            if is_prepared:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "ready",
                        "progress": 100,
                        "message": get_text("sim.already_prepared_status"),
                        "already_prepared": True,
                        "prepare_info": prepare_info
                    }
                })
            ...
            if simulation_id:
                ...
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "not_started",
                        "progress": 0,
                        "message": get_text("sim.prepare_not_started"),
                        "already_prepared": False
                    }
                })
            return jsonify({
                "success": False,
                "error": get_text("provide_task_id_or_sim_id")
            }), 400
        ...
        if not task:
                ...
                if is_prepared:
                    return jsonify({
                        ...
                        "data": {
                            ...
                            "status": "ready",
                            "progress": 100,
                            "message": get_text("sim.prepare_task_completed"),
                            "already_prepared": True,
                            "prepare_info": prepare_info
                        }
                    })
            ...
            return jsonify({
                "success": False,
                "error": get_text("task_not_found", task_id=task_id)
            }), 404
...
def get_simulation_history():
    ...
    Used for the homepage historical project display. Returns a simulation list
    enriched with project name, description, and other details.
...
def get_simulation_config(simulation_id: str):
    ...
    Returns:
        - time_config: Time settings (simulation duration, rounds, peak/off-peak periods)
    ...
    try:
        ...
        if not config:
            return jsonify({
                "success": False,
                "error": get_text("sim.config_not_found")
            }), 404
...
def download_simulation_config(simulation_id: str):
    ...
    try:
        ...
        if not os.path.exists(config_path):
            return jsonify({
                "success": False,
                "error": get_text("sim.config_file_not_found")
            }), 404
...
def download_simulation_script(script_name: str):
    ...
    try:
        ...
        if script_name not in allowed_scripts:
            return jsonify({
                "success": False,
                "error": get_text("sim.unknown_script", script_name=script_name, allowed_scripts=allowed_scripts)
            }), 400
        ...
        if not os.path.exists(script_path):
            return jsonify({
                "success": False,
                "error": get_text("sim.script_not_found", script_name=script_name)
            }), 404
...
def generate_profiles():
    ...
    try:
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": get_text("report.provide_graph_id")
            }), 400
...
def start_simulation():
    ...
    About enable_graph_memory_update:
        ...
        - This lets the graph "remember" the simulation process, for later analysis or AI conversation
    ...
    try:
        ...
        if max_rounds is not None:
            try:
                ...
                if max_rounds <= 0:
                    return jsonify({
                        "success": False,
                        "error": get_text("sim.max_rounds_positive")
                    }), 400
            except (ValueError, TypeError):
                return jsonify({
                    "success": False,
                    "error": get_text("sim.max_rounds_invalid")
                }), 400

        if platform not in ['twitter', 'reddit', 'parallel']:
            return jsonify({
                "success": False,
                "error": get_text("sim.invalid_platform", platform=platform)
            }), 400
        ...
        if state.status != SimulationStatus.READY:
            # Check if preparation is already complete
            is_prepared, prepare_info = _check_simulation_prepared(simulation_id)

            if is_prepared:
                # Preparation done, check if there's a running process
                if state.status == SimulationStatus.RUNNING:
                    ...
                    if run_state and run_state.runner_status.value == "running":
                        ...
                        if force:
                            ...
                            return jsonify({
                                "success": False,
                                "error": get_text("sim.already_running")
                            }), 400
                ...
                return jsonify({
                    "success": False,
                    "error": get_text("sim.not_ready", status=state.status.value)
                }), 400
        ...
        if enable_graph_memory_update:
            ...
            if not graph_id:
                return jsonify({
                    "success": False,
                    "error": get_text("sim.graph_memory_requires_graph")
                }), 400
...
def get_run_status_detail(simulation_id: str):
    ...
    Used by frontend to show real-time activity feed.
    ...
    try:
        ...
        # recent_actions shows only the latest round's actions from both platforms
...
def get_simulation_timeline(simulation_id: str):
    ...
    Used by frontend to display progress bar and timeline view.
...
def get_agent_stats(simulation_id: str):
    ...
    Used by frontend to display Agent activity rankings and action distribution.
    """
    try:
        ...
        if not os.path.exists(db_path):
            return jsonify({
                ...
                "data": {
                    ...
                    "count": 0,
                    "posts": [],
                    "message": get_text("sim.db_not_exists")
                }
            })
...
def interview_agent():
    ...
    (in command-wait mode after completing the simulation loop).

    Request (JSON):
        {
            "simulation_id": "sim_xxxx",    // Required, simulation ID
            "agent_id": 0,                  // Required, Agent ID
            "prompt": "What is your view?", // Required, interview question
            "platform": "twitter",          // Optional, specify platform (twitter/reddit)
                                            // If not specified: dual-platform mode interviews both platforms
    ...
    try:
        ...
        if agent_id is None:
            return jsonify({
                "success": False,
                "error": get_text("sim.provide_agent_id")
            }), 400
        
        if not prompt:
            return jsonify({
                "success": False,
                "error": get_text("sim.provide_prompt")
            }), 400
        ...
        if platform and platform not in ("twitter", "reddit"):
            return jsonify({
                "success": False,
                "error": get_text("sim.platform_invalid")
            }), 400
        ...
        if not SimulationRunner.check_env_alive(simulation_id):
            return jsonify({
                "success": False,
                "error": get_text("sim.env_not_running")
            }), 400
        ...
        return jsonify({
            "success": False,
            "error": get_text("sim.interview_timeout", error=str(e))
        }), 504
...
def interview_agents_batch():
    ...
    Request (JSON):
        {
            ...
            "interviews": [                 // Required, list of interviews
                ...
                {
                    "agent_id": 1,
                    "prompt": "What is your view on B?"  // Uses default platform if not specified
                }
            ],
    ...
    try:
        ...
        if not interviews or not isinstance(interviews, list):
            return jsonify({
                "success": False,
                "error": get_text("sim.provide_interviews")
            }), 400
        ...
        for i, interview in enumerate(interviews):
            if 'agent_id' not in interview:
                return jsonify({
                    "success": False,
                    "error": get_text("sim.interview_missing_agent_id", index=i+1)
                }), 400
            if 'prompt' not in interview:
                return jsonify({
                    "success": False,
                    "error": get_text("sim.interview_missing_prompt", index=i+1)
                }), 400
            ...
            if item_platform and item_platform not in ("twitter", "reddit"):
                return jsonify({
                    "success": False,
                    "error": get_text("sim.interview_invalid_platform", index=i+1)
                }), 400
        ...
        return jsonify({
            "success": False,
            "error": get_text("sim.batch_interview_timeout", error=str(e))
        }), 504
...
def interview_all_agents():
    ...
    Request (JSON):
        {
            "simulation_id": "sim_xxxx",                // Required, simulation ID
            "prompt": "What is your overall view?",     // Required, question (same for all agents)
            "platform": "reddit",                       // Optional, specify platform (twitter/reddit)
                                                        // If not specified: dual-platform mode interviews both
    ...
    try:
        ...
        return jsonify({
            "success": False,
            "error": get_text("sim.global_interview_timeout", error=str(e))
        }), 504
...
def get_env_status():
    ...
    try:
        ...
        if env_alive:
            message = get_text("sim.env_running")
        else:
            message = get_text("sim.env_not_available")
        ...
        logger.error(f"Failed to get env status: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
...
def close_simulation_env():
    ...
    Sends a close-environment command to the simulation for graceful exit from command-wait mode.
    ...
    try:
        ...
        logger.error(f"Failed to close env: {str(e)}")
        return jsonify({
            "success": False,
            "error": str(e),
            "traceback": traceback.format_exc()
