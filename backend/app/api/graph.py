...
def delete_project(project_id: str):
    ...
    if not success:
        return jsonify({
            "success": False,
            "error": get_text("project_delete_failed", project_id=project_id)
        }), 404

    return jsonify({
        "success": True,
        "message": get_text("project_deleted", project_id=project_id)
    })
...
def reset_project(project_id: str):
    ...
    return jsonify({
        "success": True,
        "message": get_text("project_reset", project_id=project_id),
        "data": project.to_dict()
    })
...
def generate_ontology():
    ...
    try:
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": get_text("provide_sim_requirement")
            }), 400
        ...
        if not document_texts:
            ...
            return jsonify({
                "success": False,
                "error": get_text("no_docs_processed")
            }), 400
...
def build_graph():
    ...
    try:
        ...
        if errors:
            ...
            return jsonify({
                "success": False,
                "error": get_text("config_error", errors="; ".join(errors))
            }), 500
        ...
        if project.status == ProjectStatus.CREATED:
            return jsonify({
                "success": False,
                "error": get_text("ontology_not_generated")
            }), 400

        if project.status == ProjectStatus.GRAPH_BUILDING and not force:
            return jsonify({
                "success": False,
                "error": get_text("graph_building_in_progress"),
        ...
        if not text:
            return jsonify({
                "success": False,
                "error": get_text("extracted_text_not_found")
            }), 400
        ...
        if not ontology:
            return jsonify({
                "success": False,
                "error": get_text("ontology_not_found")
            }), 400
        ...
        def build_task():
            ...
            try:
                ...
                task_manager.update_task(
                    ...
                    message=get_text("init_graph_build")
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=get_text("chunking_text"),
                    progress=5
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=get_text("creating_zep_graph"),
                    progress=10
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=get_text("setting_ontology"),
                    progress=15
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=get_text("adding_chunks", count=total_chunks),
                    progress=15
                )
                ...
                task_manager.update_task(
                    task_id,
                    message=get_text("fetching_graph_data"),
                    progress=95
                )
                ...
                task_manager.update_task(
                    ...
                    message=get_text("graph_build_complete"),
                    progress=100,
                    result={
                ...
                task_manager.update_task(
                    ...
                    message=get_text("build_failed", error=str(e)),
        ...
        return jsonify({
            ...
            "data": {
                "project_id": project_id,
                "task_id": task_id,
                "message": get_text("graph_build_started", task_id=task_id)
            }
        })
...
def delete_graph(graph_id: str):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": get_text("graph.deleted", graph_id=graph_id)
        })
