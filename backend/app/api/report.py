...
def generate_report():
    ...
    try:
        ...
        if not simulation_id:
            return jsonify({
                "success": False,
                "error": get_text("report.provide_simulation_id")
            }), 400
            ...
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "completed",
                        "message": get_text("report_exists"),
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": get_text("missing_graph_id")
            }), 400
        ...
        if not simulation_requirement:
            return jsonify({
                "success": False,
                "error": get_text("missing_sim_requirement")
            }), 400
        ...
        def run_generate():
            try:
                task_manager.update_task(
                    ...
                    progress=0,
                    message=get_text("report.init_agent")
                )
        ...
        return jsonify({
            ...
            "data": {
                ...
                "task_id": task_id,
                "status": "generating",
                "message": get_text("report.generating_started"),
...
def get_generate_status():
    ...
    try:
            ...
            if existing_report and existing_report.status == ReportStatus.COMPLETED:
                return jsonify({
                    ...
                    "data": {
                        ...
                        "status": "completed",
                        "progress": 100,
                        "message": get_text("report.report_generated"),
        ...
        if not task_id:
            return jsonify({
                "success": False,
                "error": get_text("report.provide_task_or_sim_id")
            }), 400
...
def get_report(report_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": get_text("report.not_found_id", report_id=report_id)
            }), 404
...
def get_report_by_simulation(simulation_id: str):
    ...
    try:
        ...
        if not report:
            return jsonify({
                "success": False,
                "error": get_text("report.no_report_for_sim", simulation_id=simulation_id),
                "has_report": False
            }), 404
...
def delete_report(report_id: str):
    ...
    try:
        ...
        return jsonify({
            "success": True,
            "message": get_text("report.deleted", report_id=report_id)
        })
...
def chat_with_report_agent():
    ...
    try:
        ...
        if not message:
            return jsonify({
                "success": False,
                "error": get_text("report.provide_message")
            }), 400
        ...
        if not graph_id:
            return jsonify({
                "success": False,
                "error": get_text("report.missing_graph_id")
            }), 400
...
def get_report_progress(report_id: str):
    ...
    try:
        ...
        if not progress:
            return jsonify({
                "success": False,
                "error": get_text("report.progress_not_available", report_id=report_id)
            }), 404
...
def get_single_section(report_id: str, section_index: int):
    ...
    try:
        ...
        if not os.path.exists(section_path):
            return jsonify({
                "success": False,
                "error": get_text("report.section_not_found", section_index=f"{section_index:02d}")
            }), 404
...
def get_agent_log(report_id: str):
    """
    Get detailed execution logs from the Report Agent.
    ...
    Query parameters:
        from_line: Line offset to start reading from (optional, default 0, for incremental retrieval)
...
def get_console_log(report_id: str):
    ...
    Retrieves console output (INFO, WARNING, etc.) from report generation in real-time.
...
def search_graph_tool():
    ...
    try:
        ...
        if not graph_id or not query:
            return jsonify({
                "success": False,
                "error": get_text("report.provide_graph_id_and_query")
            }), 400
