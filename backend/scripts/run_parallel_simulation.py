...
- Does not shut down the environment immediately after simulation; enters command-waiting mode
...
# This fixes OASIS third-party library file reads that don't specify encoding
# ============================================================
import sys
import os
...
def disable_oasis_logging():
    ...
    OASIS logs are too verbose (logging every agent's observations and actions); we use our own action_logger.
    """
...
def init_logging_for_simulation(simulation_dir: str):
    """
    Initialize the simulation logging configuration
...
# Available Twitter actions (excludes INTERVIEW; INTERVIEW can only be triggered via ManualAction)
TWITTER_ACTIONS = [
...
# Available Reddit actions (excludes INTERVIEW; INTERVIEW can only be triggered via ManualAction)
REDDIT_ACTIONS = [
    ActionType.LIKE_POST,
...
class ParallelIPCHandler:
    ...
    async def _interview_single_platform(self, agent_id: int, prompt: str, platform: str) -> Dict[str, Any]:
        ...
        Returns:
            Dict containing results, or dict containing error
        """
    ...
    async def handle_interview(self, command_id: str, agent_id: int, prompt: str, platform: str = None) -> bool:
        ...
        Args:
            ...
            platform: Target platform (optional)
                ...
                - None/unspecified: Interview on both platforms and return merged result
    ...
    async def handle_batch_interview(self, command_id: str, interviews: List[Dict], platform: str = None) -> bool:
        ...
        # Split both_platforms_interviews across the two platforms
    ...
    def _get_interview_result(self, agent_id: int, platform: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # Query the most recent interview records
            cursor.execute("""
...
# Non-core action types to filter out (these have lower analytical value)
...
def fetch_new_actions_from_db(
    ...
    """
    Get new action records from the database and enrich them with full context information
    
    Args:
        db_path: Database file path
        last_rowid: Maximum rowid from last read (using rowid instead of created_at because different platforms use different created_at formats)
    ...
    Returns:
        ...
        - new_last_rowid: New maximum rowid
    """
    actions = []
    ...
    try:
        ...
        # This avoids created_at format discrepancies (Twitter uses integer, Reddit uses datetime string)
        cursor.execute("""
        ...
        for rowid, user_id, action, info_json in cursor.fetchall():
            ...
            # Trim action_args to key fields only (preserve full content, no truncation)
            simplified_args = {}
...
def _enrich_action_context(
    ...
    try:
        ...
        # Retweet/repost: add original post content and author
        elif action_type == 'REPOST':
            new_post_id = action_args.get('new_post_id')
            if new_post_id:
                # For retweets, original_post_id points to the original post
                cursor.execute("""
        ...
        # Quote post: add original post content, author, and quote comment
            ...
            # Get the comment content for the quoted post (quote_content)
            if new_post_id:
                cursor.execute("""
        ...
        # Post comment: add the post info being commented on
...
def create_model(config: Dict[str, Any], use_boost: bool = False):
    ...
    If a boost LLM is configured, parallel simulation can use different API providers for each platform, improving concurrency.
    ...
    # Select which LLM to use based on parameters and config availability
    ...
    # Fall back to config if model name is not in .env
    if not llm_model:
    ...
    if not os.environ.get("OPENAI_API_KEY"):
        raise ValueError("Missing API key config. Please set LLM_API_KEY in the .env file at the project root")
...
async def run_twitter_simulation(
    ...
    Args:
        ...
        max_rounds: Maximum number of rounds (optional, used to truncate long simulations)
    ...
    # Get real agent name mapping from config (using entity_name instead of the default Agent_X)
    agent_names = get_agent_names_from_config(config)
    # Fall back to OASIS default names for agents not in the config
    ...
    total_actions = 0
    last_rowid = 0  # Track the last processed rowid in the database (using rowid avoids created_at format differences)
    ...
    # Truncate if a maximum round count is specified
    ...
    for round_num in range(total_rounds):
        ...
        # Always log round start, regardless of active agents
        if action_logger:
        ...
        # Read actually executed actions from the database and log them
...
async def main():
    ...
    parser.add_argument(
        ...
        action='store_true',
        default=False,
        help='Shut down the environment immediately after simulation completes, do not enter command-waiting mode'
    )
    
    args = parser.parse_args()
    
    # Create shutdown event at the start of main to ensure the whole program can respond to exit signals
    ...
    if wait_for_commands:
        log_manager.info("")
        log_manager.info("=" * 60)
        log_manager.info("Entering command-waiting mode - environment staying open")
        ...
        try:
            while not _shutdown_event.is_set():
                ...
                if not should_continue:
                    break
                # Use wait_for instead of sleep to be responsive to shutdown_event
                try:
                    await asyncio.wait_for(_shutdown_event.wait(), timeout=0.5)
                    break  # Received exit signal
                except asyncio.TimeoutError:
                    pass  # Timed out, continue loop
...
def setup_signal_handlers(loop=None):
    ...
    def signal_handler(signum, frame):
        ...
        # Do not call sys.exit() directly; let the asyncio loop exit and clean up resources normally
