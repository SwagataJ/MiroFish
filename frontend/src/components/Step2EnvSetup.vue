<template>
  <div class="env-setup-panel">
    <div class="scroll-container">
        ...
        <div class="card-content">
          ...
          <div v-if="profiles.length > 0" class="stats-grid">
            <div class="stat-card">
              ...
              <span class="stat-label">{{ $t('step2.entityCount') }}</span>
            </div>
            <div class="stat-card">
              <span class="stat-value">{{ expectedTotal || '-' }}</span>
              <span class="stat-label">{{ $t('step2.personaCount') }}</span>
            </div>
            <div class="stat-card">
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        <div class="card-header">
          <div class="step-info">
            ...
            <span class="step-title">{{ $t('step2.simConfig') }}</span>
          </div>
          <div class="step-status">
          ...
          <div v-if="simulationConfig" class="config-detail-panel">
            ...
            <div class="config-block">
              <div class="config-grid">
                ...
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.minutesPerRoundLabel') }}</span>
                  <span class="config-item-value">{{ simulationConfig.time_config?.minutes_per_round || '-' }} {{ $t('step2.minutesUnit') }}</span>
                </div>
                <div class="config-item">
                  <span class="config-item-label">{{ $t('step2.maxRoundsLabel') }}</span>
            ...
            <div class="config-block">
              <div class="config-block-header">
                ...
                <span class="config-block-badge">{{ $t('step2.agentCountUnit', { count: simulationConfig.agent_configs?.length || 0 }) }}</span>
              </div>
      ...
      <!-- Step 05: Ready Complete -->
      <div class="step-card" :class="{ 'active': phase === 4 }">
        <div class="card-header">
          <div class="step-info">
            <span class="step-num">05</span>
            <span class="step-title">{{ $t('step2.readyComplete') }}</span>
          </div>
          <div class="step-status">
        ...
        <div class="card-content">
          ...
          <p class="description">{{ $t('step2.simReadyDesc') }}</p>
          
          <!-- Simulation rounds config - only show when config is generated and rounds calculated -->
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                <span class="section-title">{{ $t('step2.roundsConfigTitle') }}</span>
                <span class="section-desc" v-html="$t('step2.roundsConfigDesc', { hours: `<span class='desc-highlight'>${simulationConfig?.time_config?.total_simulation_hours || '-'}</span>`, minutes: `<span class='desc-highlight'>${simulationConfig?.time_config?.minutes_per_round || '-'}</span>` })"></span>
              </div>
              <label class="switch-control">
                <input type="checkbox" v-model="useCustomRounds">
                <span class="switch-track"></span>
                <span class="switch-label">{{ $t('step2.customMode') }}</span>
              </label>
            </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                ...
                <div class="range-wrapper">
                  ...
                  <div class="range-marks">
                    ...
                    >{{ $t('step2.recommendedMark', { rounds: 40 }) }}</span>
                  ...
                  <div class="auto-content">
                    <div class="auto-meta-row">
                      <span class="duration-badge">
                        ...
                        {{ $t('step2.autoEstimatedTime', { minutes: Math.round(autoGeneratedRounds * 0.6) }) }}
                      </span>
                    </div>
                    <div class="auto-desc">
        ...
        <div class="modal-body">
          ...
          <div class="modal-info-grid">
            <div class="info-item">
              ...
              <span class="info-value">{{ selectedProfile.age ? $t('step2.yearsOld', { age: selectedProfile.age }) : '-' }}</span>
            </div>
            <div class="info-item">
          ...
          <div class="modal-section">
            ...
            <p class="section-bio">{{ selectedProfile.bio || $t('step2.noBioAvailable') }}</p>
          </div>

          <!-- Interested topics -->
          <div class="modal-section" v-if="selectedProfile.interested_topics?.length">
            <span class="section-label">{{ $t('step2.relatedTopics') }}</span>
          ...
          <div class="modal-section" v-if="selectedProfile.persona">
            ...
            <div class="persona-dimensions">
              ...
              <div class="dimension-card">
                <span class="dim-title">{{ $t('step2.dimUniqueMemory') }}</span>
                <span class="dim-desc">{{ $t('step2.dimUniqueMemoryDesc') }}</span>
              </div>
              <div class="dimension-card">
...
// Log dedup: track last logged info
...
watch(currentStage, (newStage) => {
  if (newStage === 'generating_profiles') {
    ...
    if (!configTimer) {
      addLog(t('step2.logGenDualPlatformConfig'))
      startConfigPolling()
    }
...
const autoGeneratedRounds = computed(() => {
  ...
  // Ensure max rounds >= 40 (recommended) to prevent slider range issues
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    ...
    addLog(t('step2.logStartSimCustom', { rounds: customMaxRounds.value }))
  } else {
    addLog(t('step2.logStartSimAuto', { rounds: autoGeneratedRounds.value }))
  }
...
const startPrepareSimulation = async () => {
  ...
  phase.value = 1
  addLog(t('step2.logSimCreated', { id: props.simulationId }))
  addLog(t('step2.logPreparingEnv'))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.already_prepared) {
        addLog(t('step2.logAlreadyPrepared'))
        await loadPreparedData()
        return
      }
      
      taskId.value = res.data.task_id
      addLog(t('step2.logPrepareTaskStarted'))
      addLog(`  └─ Task ID: ${res.data.task_id}`)

      // Set expected agent total from prepare API response
      if (res.data.expected_entities_count) {
        expectedTotal.value = res.data.expected_entities_count
        addLog(t('step2.logEntitiesRead', { count: res.data.expected_entities_count }))
        if (res.data.entity_types && res.data.entity_types.length > 0) {
          addLog(`  └─ ${t('step2.logEntityTypes', { types: res.data.entity_types.join(', ') })}`)
        }
      }

      addLog(t('step2.logPollingProgress'))
      // Start polling progress
      startPolling()
      // Start realtime profiles fetching
      startProfilesPolling()
    } else {
      addLog(t('step2.logPrepareFailed', { error: res.error || 'Unknown error' }))
      emit('update-status', 'error')
    }
  } catch (err) {
    addLog(t('step2.logPrepareError', { error: err.message }))
    emit('update-status', 'error')
  }
}
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.status === 'completed' || data.status === 'ready' || data.already_prepared) {
        addLog(`✓ ${t('step2.logPrepareComplete')}`)
        stopPolling()
        stopProfilesPolling()
        await loadPreparedData()
      } else if (data.status === 'failed') {
        addLog(`✗ ${t('step2.logPrepareFinalFailed', { error: data.error || 'Unknown error' })}`)
        stopPolling()
        stopProfilesPolling()
      }
    }
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (currentCount > 0 && currentCount !== lastLoggedProfileCount) {
        ...
        if (currentCount === 1) {
          addLog(t('step2.logStartGenProfiles'))
        }
        addLog(`→ ${t('step2.logProfileProgress', { current: currentCount, total, name: profileName, profession: latestProfile?.profession || t('step2.logUnknownProfession') })}`)

        // If all generated
        if (expectedTotal.value && currentCount >= expectedTotal.value) {
          addLog(`✓ ${t('step2.logAllProfilesDone', { count: currentCount })}`)
        }
      }
    }
  } catch (err) {
    ...
    if (res.success && res.data) {
      ...
      if (data.generation_stage && data.generation_stage !== lastLoggedConfigStage) {
        ...
        if (data.generation_stage === 'generating_profiles') {
          addLog(t('step2.logGenProfileConfig'))
        } else if (data.generation_stage === 'generating_config') {
          addLog(t('step2.logCallingLLMConfig'))
        }
      }
      ...
      if (data.config_generated && data.config) {
        ...
        addLog(`✓ ${t('step2.logConfigComplete')}`)

        // Show detailed config summary
        if (data.summary) {
          addLog(`  ├─ ${t('step2.logAgentCount', { count: data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.logSimDuration', { hours: data.summary.simulation_hours })}`)
          addLog(`  ├─ ${t('step2.logInitialPosts', { count: data.summary.initial_posts_count })}`)
          addLog(`  ├─ ${t('step2.logHotTopics', { count: data.summary.hot_topics_count })}`)
          addLog(`  └─ ${t('step2.logPlatformConfig', { twitter: data.summary.has_twitter_config ? '✓' : '✗', reddit: data.summary.has_reddit_config ? '✓' : '✗' })}`)
        }

        // Show time config details
        if (data.config.time_config) {
          const tc = data.config.time_config
          addLog(t('step2.logTimeConfig', { minutes: tc.minutes_per_round, rounds: Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round) }))
        }

        // Show event config
        if (data.config.event_config?.narrative_direction) {
          const narrative = data.config.event_config.narrative_direction
          addLog(t('step2.logNarrativeDirection', { text: narrative.length > 50 ? narrative.substring(0, 50) + '...' : narrative }))
        }

        stopConfigPolling()
        phase.value = 4
        addLog(`✓ ${t('step2.logEnvComplete')}`)
...
const loadPreparedData = async () => {
  phase.value = 2
  addLog(t('step2.logLoadingConfig'))

  // Final fetch of profiles
  await fetchProfilesRealtime()
  addLog(t('step2.logLoadedProfiles', { count: profiles.value.length }))
  ...
  try {
    ...
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        addLog(`✓ ${t('step2.logConfigLoadSuccess')}`)

        // Show detailed config summary
        if (res.data.summary) {
          addLog(`  ├─ ${t('step2.logAgentCount', { count: res.data.summary.total_agents })}`)
          addLog(`  ├─ ${t('step2.logSimDuration', { hours: res.data.summary.simulation_hours })}`)
          addLog(`  └─ ${t('step2.logInitialPosts', { count: res.data.summary.initial_posts_count })}`)
        }
        ...
        addLog(t('step2.logConfigGenerating'))
        startConfigPolling()
      }
    }
  } catch (err) {
    addLog(t('step2.logConfigLoadFailed', { error: err.message }))
    emit('update-status', 'error')
  }
}
