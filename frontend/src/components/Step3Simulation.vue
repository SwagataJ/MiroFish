    ...
    <div class="control-bar">
      ...
      <div class="action-controls">
        ...
        >
          ...
          {{ isGeneratingReport ? $t('step3.startingReport') : $t('step3.generateReport') }}
...
const doStartSimulation = async () => {
  if (!props.simulationId) {
    addLog(t('step3.errorMissingSimIdStart'))
    return
  }
  ...
  startError.value = null
  addLog(t('step3.startingDualPlatform'))
  ...
  try {
    ...
    if (props.maxRounds) {
      ...
      addLog(t('step3.settingMaxRounds', { rounds: props.maxRounds }))
    }
    ...
    if (res.success && res.data) {
      if (res.data.force_restarted) {
        addLog(`✓ ${t('step3.oldSimCleared')}`)
      }
      addLog(`✓ ${t('step3.simEngineStarted')}`)
      ...
      addLog(`✗ ${t('step3.startFailed', { error: res.error || t('step3.unknownError') })}`)
...
const handleStopSimulation = async () => {
  ...
  try {
    ...
    if (res.success) {
      ...
      addLog(t('step3.stopFailed', { error: res.error || t('step3.unknownError') }))
    }
  } catch (err) {
...
const fetchRunStatus = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Extra check: if backend hasn't updated runner_status yet but platforms report done
...
const checkPlatformsCompleted = (data) => {
  ...
  // Check if platform is enabled (count > 0 or running was true)
...
const handleNextStep = async () => {
  ...
  addLog(t('step3.startingReportGen'))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(`✗ ${t('step3.reportStartFailed', { error: res.error || t('step3.unknownError') })}`)
