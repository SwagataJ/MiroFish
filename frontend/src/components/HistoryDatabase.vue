        ...
        <div v-if="selectedProject" class="modal-overlay" @click.self="closeModal">
          <div class="modal-content">
            ...
            <div class="modal-divider">
              ...
              <span class="divider-text">{{ $t('history.simPlayback') }}</span>
              <span class="divider-line"></span>
            </div>
...
const getCardStyle = (index) => {
  ...
  if (isExpanded.value) {
    ...
    // Calculate cards in current row, ensure each row is centered
...
const getSimulationTitle = (requirement) => {
  if (!requirement) return t('history.untitledSim')
  ...
  observer = new IntersectionObserver(
    (entries) => {
      entries.forEach((entry) => {
        ...
        // Update pending target state (always record latest target regardless of animation state)
        ...
        // Expand delay is short (50ms), collapse delay is longer (200ms) for stability
...
/* Simplified display when no projects exist */
...
.grid-pattern {
  ...
  /* Anchored from top-left; height changes only extend the grid downward, preserving existing grid positions */
