        ...
        <div v-if="activeTab === 'survey'" class="survey-container">
          ...
          <div v-if="surveyResults.length > 0" class="survey-results">
            <div class="results-header">
              ...
              <span class="results-count">{{ $t('step5.repliesCount', { count: surveyResults.length }) }}</span>
            </div>
...
const selectAgent = (agent, idx) => {
  ...
  addLog(t('step5.selectedChatTarget', { name: agent.username }))
}
...
const renderMarkdown = (content) => {
  ...
  // Clean up <br> before paragraph opening tags that follow block-level elements
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-consecutive ordered list numbering: maintain incrementing numbers when single-item <ol> blocks are separated by paragraph content
...
const sendMessage = async () => {
  ...
  try {
    ...
    addLog(t('step5.sendFailed', { error: err.message }))
    chatHistory.value.push({
      role: 'assistant',
...
const sendToReportAgent = async (message) => {
  addLog(t('step5.sentToReportAgent', { message: message.substring(0, 50) }))
  ...
  if (res.success && res.data) {
    ...
    addLog(t('step5.reportAgentReplied'))
  } else {
...
const sendToAgent = async (message) => {
  ...
  if (chatHistory.value.length > 1) {
    const historyContext = chatHistory.value
      ...
      .join('\n')
    prompt = t('step5.previousConversation', { history: historyContext, message: message })
  }
...
const submitSurvey = async () => {
  ...
  addLog(t('step5.surveySentTo', { count: selectedAgents.value.size }))
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.receivedReplies', { count: surveyResults.value.length }))
    } else {
      throw new Error(res.error || t('step5.requestFailed'))
    }
  } catch (err) {
    addLog(t('step5.surveySendFailed', { error: err.message }))
  } finally {
...
const loadReportData = async () => {
  ...
  try {
    addLog(t('step5.loadingReportData', { id: props.reportId }))
    ...
    addLog(t('step5.loadReportFailed', { error: err.message }))
  }
}
...
const loadAgentLogs = async () => {
  ...
  try {
    ...
    addLog(t('step5.loadReportLogsFailed', { error: err.message }))
  }
}
...
const loadProfiles = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      addLog(t('step5.loadedProfiles', { count: profiles.value.length }))
    }
  } catch (err) {
    addLog(t('step5.loadProfilesFailed', { error: err.message }))
  }
}
