  ...
  <div class="graph-panel">
    <div class="panel-header">
      ...
      <div class="header-tools">
        ...
        <button class="tool-btn" @click="$emit('toggle-maximize')" :title="$t('graphPanel.maximizeRestore')">
    ...
    <div class="graph-container" ref="graphContainer">
      ...
      <div v-if="graphData" class="graph-view">
        ...
        <div v-if="currentPhase === 1 || isSimulating" class="graph-building-hint">
          ...
          {{ isSimulating ? $t('graphPanel.realtimeUpdating') : $t('graphPanel.buildingUpdating') }}
        </div>
        ...
        <div v-if="showSimulationFinishedHint" class="graph-building-hint finished-hint">
          ...
          <span class="hint-text">{{ $t('graphPanel.pendingContent') }}</span>
...
const renderGraph = () => {
  ...
  // Simulation - dynamically adjust node distance based on edge count
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // Dynamically adjust distance based on edge count between node pair
      // Base distance 150, add 50 per additional edge
    ...
    // Add center gravity to keep isolated node groups near center
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      ...
      .on('drag', (event, d) => {
        // Detect if drag actually started (moved past threshold)
      ...
      .on('end', (event, d) => {
        // Only let simulation cool down if actually dragged
        if (d._isDragging) {
