            ...
            >
              ...
              <div class="timeline-content">
                ...
                <div class="timeline-body" :class="{ 'collapsed': isLogCollapsed(log) }" @click="toggleLogExpand(log)">
                  ...
                  <!-- Section Content Generated (content generated, but whole section may not be complete yet) -->
...
const parseInsightForge = (text) => {
  ...
  try {
    ...
    const reqMatch = text.match(/Simulation scenario:\s*(.+?)(?:\n|$)/)
...
const parseInterview = (text) => {
  ...
  try {
    ...
    const reasonMatch = text.match(/### Agent Selection Reasoning\n([\s\S]*?)(?=\n---\n|\n### Interview Transcript)/)
    if (reasonMatch) {
    ...
    const parseIndividualReasons = (reasonText) => {
      ...
      for (const line of lines) {
        ...
        if (!headerMatch) {
          headerMatch = line.match(/^-\s*Select\s+([^（(]+)(?:[（(]index\s*=?\s*\d+[)）])?[：:]\s*(.*)/)
          if (headerMatch) {
            name = headerMatch[1].trim()
            reasonStart = headerMatch[2]
          }
        ...
        } else if (currentName && line.trim() && !line.match(/^In summary|^Overall|^Finally/i)) {
    ...
    interviewBlocks.forEach((block, index) => {
      ...
      if (answerMatch) {
        ...
        if (!twitterMatch && redditMatch) {
          ...
          // No platform markers (very old format), use whole text as answer
      ...
      h('div', { class: 'insight-content' }, [
        ...
        activeTab.value === 'facts' && props.result.facts.length > 0 && h('div', { class: 'facts-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, t('step4.latestKeyFactsInTemporalMemory')),
          ...
          }, expandedFacts.value ? t('step4.collapse') : t('step4.expandAllItems', { count: props.result.facts.length }))
        ]),
        ...
        activeTab.value === 'relations' && props.result.relations.length > 0 && h('div', { class: 'relations-panel' }, [
          ...
          }, expandedRelations.value ? t('step4.collapse') : t('step4.expandAllItems', { count: props.result.relations.length }))
        ]),

        // Sub-queries Tab
        activeTab.value === 'subqueries' && props.result.subQueries.length > 0 && h('div', { class: 'subqueries-panel' }, [
          h('div', { class: 'panel-header' }, [
            h('span', { class: 'panel-title' }, t('step4.driftQuerySubQueries')),
    ...
    return () => h('div', { class: 'panorama-display' }, [
      ...
      h('div', { class: 'panorama-content' }, [
        ...
        activeTab.value === 'active' && h('div', { class: 'facts-panel active-facts' }, [
          ...
          }, expandedActive.value ? t('step4.collapse') : t('step4.expandAllItems', { count: props.result.activeFacts.length }))
        ]),
        ...
        activeTab.value === 'historical' && h('div', { class: 'facts-panel historical-facts' }, [
          ...
          }, expandedHistorical.value ? t('step4.collapse') : t('step4.expandAllItems', { count: props.result.historicalFacts.length }))
        ]),
...
const InterviewDisplay = {
  ...
  setup(props) {
    ...
    const splitAnswerByQuestions = (answerText, questionCount) => {
      ...
      // 1. "Question X:" (English format, current backend format)
      // 2. "1. " or "\n1. " (number+dot, legacy format compatible)
      let matches = []
      let match

      // Prefer "Question X:" format first
      const enPattern = /(?:^|[\r\n]+)Question\s*(\d+)[：:]\s*/gi
...
// Currently active step (used for top panel display)
...
const renderMarkdown = (content) => {
  ...
  // Remove leading h2 title (## xxx) since section title is already displayed in the outer container
  ...
  // Clean <p><br> immediately followed by block-level elements (caused by extra blank lines)
  html = html.replace(/<p class="md-p">(<br>\s*)+(<ul|<ol|<blockquote|<pre|<hr)/g, '$2')
  // Clean consecutive <br> tags
  html = html.replace(/(<br>\s*){2,}/g, '<br>')
  // Clean <br> between block-level closing tags and following paragraph opening tags
  html = html.replace(/(<\/ol>|<\/ul>|<\/blockquote>)<br>(<p|<div)/g, '$1$2')

  // Fix non-contiguous ordered list numbering: when single-item <ol> blocks are separated by paragraph content, maintain incrementing numbers
      ...
      if (newLogs.length > 0) {
        newLogs.forEach(log => {
          ...
          if (log.action === 'report_complete') {
            ...
            stopPolling()
            // Scroll logic is handled in the nextTick after the loop ends
          }
...
const extractFinalContent = (response) => {
  ...
  // Try alternate "Answer:" marker
  const altAnswerMatch = response.match(/Answer:\s*\n*([\s\S]*)$/i)
  if (altAnswerMatch) {
    return altAnswerMatch[1].trim()
  }
  ...
  // If content is long and contains markdown formatting, try removing the thinking process and return
