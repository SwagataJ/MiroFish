    ...
    <div class="main-content">
      ...
      <div class="left-panel" :class="{ 'full-screen': isFullScreen }">
        ...
        <div class="graph-container" ref="graphContainer">
          <!-- Graph visualization (show as long as data exists) -->
          <div v-if="graphData" class="graph-view">
            <svg ref="graphSvg" class="graph-svg"></svg>
            <!-- Building hint -->
            <div v-if="currentPhase === 1" class="graph-building-hint">
              <span class="building-dot"></span>
              {{ $t('process.updatingInRealtime') }}
            </div>
          ...
          <div v-else-if="currentPhase < 1" class="graph-waiting">
            ...
            <p class="waiting-text">{{ $t('process.waitingForOntology') }}</p>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 0, 'completed': currentPhase > 0 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="!projectData?.ontology && currentPhase === 0 && !ontologyProgress">
                <div class="waiting-hint">{{ $t('process.waitingForOntologyGeneration') }}</div>
              </div>
            </div>
          </div>
          ...
          <div class="process-phase" :class="{ 'active': currentPhase === 1, 'completed': currentPhase > 1 }">
            ...
            <div class="phase-detail">
              ...
              <div class="detail-section waiting-state" v-if="currentPhase < 1">
                <div class="waiting-hint">{{ $t('process.waitingForOntologyCompletion') }}</div>
              </div>
...
const statusText = computed(() => {
  ...
  if (currentPhase.value === 0) return t('process.statusOntologyGenerating')
...
const loadProject = async () => {
  try {
    ...
    if (response.success) {
      ...
      // Continue polling for in-progress build tasks
    ...
    error.value = t('process.loadProjectFailed') + ': ' + (err.message || t('process.unknownError'))
  } finally {
    loading.value = false
  }
}
...
const renderGraph = () => {
  ...
  if (nodesData.length === 0) {
    ...
    svg.append('text')
      ...
      .attr('fill', '#999')
      .text(t('process.waitingForGraphData'))
    return
  }
  ...
  simulation.on('tick', () => {
    // Update all edge positions (both visible lines and transparent click areas)
